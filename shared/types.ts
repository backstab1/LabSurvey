...
/** Атрибут конджойнта: например «Цена» с уровнями 199 / 249 / 299 ₽. У уровня может быть картинка (image), текст можно скрыть (hideText) */
export interface ConjointAttribute {
  id: string;
  text: string;
  levels: Option[];
  /** Показывать сверху карточки крупно (сервис, бренд с логотипом) — вместо строки таблицы */
  header?: boolean;
  /** Закреплён за карточкой: карточка 1 — первый уровень, карточка 2 — второй… (число уровней = число карточек) */
  fixed?: boolean;
}
